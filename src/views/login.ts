namespace view {
  ...
  export class LoginVentana {
    ...
    constructor(wrapper: controller.VentanaContenedor, onRegistro?: () => void, onBienvenida?: () => void) {
    ...
    private enviarCorreo() {
      this.correoUsuario = this.inputEmail.node()?.value?.trim() || "";
    ...
    private mostrarLoginPassword() {
      ...
      const inputBox = inputPassword.node().parentNode as HTMLElement;
    ...
    private mostrarFaceID() {
      ...
      const scanFace = async () => {
        if (peticionEnCurso) return; // Ignorar si ya hay petición en curso
        ...
        controller.ApiService.loginBiometrico(this.correoUsuario, descriptor, (success, result) => {
          ...
          if (success && result && result.token) {
            ...
            const iconElement = faceIcon.node() as HTMLElement;
    ...
    private mostrarLoginSMS() {
      ...
      const btnNext = this.control.navDobleBotones(this.contenedor, {
        ...
        nextLabel: "Verificar",
        onNext: () => {
          const code = inputCode.node()?.value.trim();
          ...
          controller.ApiService.loginConToken(this.correoUsuario, "sms", code, (success, data) => {
            if (success && data?.token) {
              ...
              status.text(data?.message || "Código incorrecto").style("color", "#ff6b6b");
            }
          });
        },
        nextDisabled: true
    ...
    private mostrarLoginEmailToken() {
      this.contenedor.html("");
      ...
      const btnNext = this.control.navDobleBotones(this.contenedor, {
        ...
        nextLabel: "Verificar",
        onNext: () => {
          const token = inputToken.node()?.value.trim();
          ...
          controller.ApiService.loginConToken(this.correoUsuario, "correo", token, (success, data) => {
            if (success && data?.token) {
              ...
              status.text(data?.message || "Token incorrecto").style("color", "#e74c3c");
            }
          });
        },
        nextDisabled: true
